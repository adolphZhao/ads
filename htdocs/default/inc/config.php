<?php
error_reporting( E_ERROR);
$config = array();
$config['report'] = 'http://mp.weixin.qq.com/mp/infringement?username=gh_70d308847e1f&from=1#wechat_redirect';
$config['password'] = 'Aa123456...';
$config['encode_key'] = 'DCWD';
$config['tj'] = true;
$config['check-api'] = 'http://vip.weixin139.com/weixin/ll0358a.php?domain={domain}';
$config['platform']['username']     = 'cdcf8234@163.com';
$config['platform']['password']     = 'Aa123456...';
$config['platform']['AESKEY']       = '63c5d6361efc548ae2dbce2658dc71be6129d87ff82';
$config['platform']['TOKEN']        = '7a94fb5edbc2e8aedb3be36daaaf354b';

$config['gateway_res'] = '//jiandingshu.cn/p';
